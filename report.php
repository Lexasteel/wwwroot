...
if ($_GET['PeriodType'] == 2) {
    $start = "01." . $_GET['MY_Month'] . "." . $_GET['MY_Year'];
    if ($_GET['MY_Month'] < 12)
        $end = "01." . ($_GET['MY_Month'] + 1) . "." . $_GET['MY_Year'];
    else
        $end = "01.01." . ($_GET['MY_Year'] + 1);
}
if ($_GET['PeriodType'] == 3) {
    $start = "01." . (1 + ($_GET['QY_Quartal'] - 1) * 3) . "." . $_GET['QY_Year'];
    $end = "01." . (4 + ($_GET['QY_Quartal'] - 1) * 3) . "." . $_GET['QY_Year'];
}
if ($_GET['PeriodType'] == 4) {
    $start = "01.01." . $_GET['Year'];
    $end = "01.01." . ($_GET['Year'] + 1);
}

if ($unitSelect == 'daily') {
    $startmmsshh = " 00:00:00";
    $endmmsshh = " 23:59:59";
} else {
    $startmmsshh = " 00:00:01";
    $endmmsshh = " 00:00:00";

    if ($PeriodType == 0 || $PeriodType == 1)
        $end = date("d.m.Y", strtotime($end) + 60 * 60 * 24);

}

$start = $start . $startmmsshh;
$end = $end . $endmmsshh;
